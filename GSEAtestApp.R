...
# Gene Set Data
gmt <- read.gmt("~/R/ALCLProject_test/testApp/testerapp/Up_Reg_Myeloid_Cells_Gruber_Lab.gmt")
colnames(gmt)[1] <- "ont"
gmt$ont <- gsub("MEP", "Gruber_Lab_MEP", gmt$ont)
# Expression Data
expr_data <- read.table("~/R/ALCLProject_test/testApp/testerapp/BM_Mouse_Model_TPM_20210816_reorder.max.txt", sep="\t", header=T, row.names=1, quote="")
...
server <- function(input, output, session) {
  ...
  output$LeadingEdgeGenes <- DT::renderDataTable({
    ...
    DT::datatable(GeneSymbol, options = list(paging = F), caption = "Leading Edge Genes",  rownames = F)
  })
  ...
  output$downloadGeneList <- downloadHandler({
    ...
    GeneSymbol <- GeneSymbol[,c("Rank","GeneSymbol")]
    filename = function() {
      paste(GeneSymbol,"_",Sys.Date(),".csv",sep = "")
    }
